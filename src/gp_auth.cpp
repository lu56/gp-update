...
bool hasValidTokenCache(const AppConfig& config) {
    if (config.authToken.empty() || config.authExpire.empty()) return false;
    // Parse "yyyy-MM-dd HH:mm:ss"
    struct tm lt = {};
    if (sscanf_s(config.authExpire.c_str(), "%d-%d-%d %d:%d:%d",
                 &lt.tm_year, &lt.tm_mon, &lt.tm_mday,
                 &lt.tm_hour, &lt.tm_min, &lt.tm_sec) != 6) return false;
    lt.tm_year -= 1900;
    lt.tm_mon -= 1;
